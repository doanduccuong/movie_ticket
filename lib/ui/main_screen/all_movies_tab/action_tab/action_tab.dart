...
class ActionTab extends StatefulWidget {
  const ActionTab({Key? key}) : super(key: key);
  @override
    ...
    return BlocBuilder<AppCubit,CubitStates>(
      builder: (context, state) {
        if (state is LoadedState) {
          ...
          List<DataModel> actionList=[

          ];
          for(int i=0;i<httpData.length;i++)
            {
              httpData[i].genreIds!.contains(28)?actionList.add(httpData[i]):null;

            }
          print(actionList[3].title);
          return Column(
            children: [
              Container(
                    ...
                    onTap: _focusIndex == index
                        ? () {
                            ...
                            BlocProvider.of<AppCubit>(context)
                                .getDetailData(actionList[index]);
                          }
                        : null,
                    child: Container(
                      height: 201.64.h,
              ...
              Center(
                child: TextNormal(
                  title: actionList[_focusIndex].originalTitle!,
