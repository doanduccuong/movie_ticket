    ...
    return  BlocBuilder<AppCubit,CubitStates>(
      builder: (context, state) {
        if (state is LoadedState) {
          ...
          List<DataModel> fantasyList=[

          ];
          for(int i=0;i<httpData.length;i++)
          {
            httpData[i].genreIds!.contains(14)?fantasyList.add(httpData[i]):null;

          }
          print(fantasyList[3].title);
          return Column(
            children: [
              Container(
                  ...
                  itemBuilder: (context, index) => InkWell(
                    ...
                    onTap: _focusIndex == index
                      ...
                      BlocProvider.of<AppCubit>(context)
                          .getDetailData(fantasyList[index]);
                    }
                        : null,
                    child: Container(
                      height: 201.64.h,
                        ...
                        image: DecorationImage(
                            image: NetworkImage(
                                "https://image.tmdb.org/t/p/w500" +
                                    fantasyList[index].backdropPath!),
                            fit: BoxFit.cover),
                      ),
                    ),
                  ),
              ...
              Center(
                child: TextNormal(
                  title: fantasyList[_focusIndex].originalTitle!,
